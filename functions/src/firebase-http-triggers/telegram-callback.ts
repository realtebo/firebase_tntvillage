  ...
  switch (text) {
    case "/refresh": {
      ...
      if (id !== MIRKO) {
        await sendTo(
          MIRKO,
          "Mittente non approvato per questo comando: " + JSON.stringify(from)
        );
        await sendTo(
          id,
          "Spiacente " +
            ...
            " " +
            last_name +
            ", solo alcuni utenti posso eseguire questo comando"
        );
        return;
      }
      ...
      await sendTo(MIRKO, "Refresh avviato");
      if (!result) {
    ...
    default: {
      await sendTo(
        MIRKO,
        "Comando sconosciuto: " + text + " da " + first_name + " " + last_name
      );
      await sendTo(
        id,
        "Spiacente " +
...
const splitByEquals = (string): Array<string> =>{
    return _.split(string, "=", 2);
}
...
export const telegram_callback = async (
  ...
  if (body.callback_query) {
    ...
    const query = _.chain(body.callback_query.data)
      ...
      .map(splitByEquals) // [["a","b454"],["c","dhjjh"],["f","g6hksdfjlksd"]]
    ...
    switch (query.command) {
      case "is_english":
        ...
        break;
      case "delete_message":
        const result: DeleteResult = await deleteMessageFromChat(
          chat_id,
          message_id
        );
        if (result.success === false) {
          await sendTo(
            from_id,
            "Messaggio non cancellabile, forse è troppo vecchio\n"
          );
        }
        break;
      case "delete_show":
      ...
      default:
        await sendTo(
          MIRKO,
          "Callback query non implementata\n" + util.inspect(query)
        );
    }
